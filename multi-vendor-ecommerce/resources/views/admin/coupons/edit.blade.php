...
@section('content')
        <div class="flex items-center justify-between">
            <h2 class="font-extrabold text-2xl text-gray-800 leading-tight">
                {{ __('Edit Coupon:') }} <span class="text-emerald-600">{{ $coupon->code }}</span>
            </h2>
                        ...
                        <div class="bg-white  shadow-sm border border-gray-100 rounded-3xl">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="col-span-2">
                                    ...
                                    <x-text-input id="code" name="code" type="text" class="block w-full mt-1 rounded-xl border-gray-200 focus:ring-emerald-500" :value="old('code', $coupon->code)" required />
                                    <x-input-error :messages="$errors->get('code')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="type" :value="__('Discount Type')" class="text-xs font-bold uppercase text-gray-500 mb-2" />
                                    <select name="type" id="type" class="block w-full mt-1 rounded-xl border-gray-200 text-sm focus:ring-emerald-500">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="value" name="value" type="number" step="0.01" class="block w-full mt-1 rounded-xl border-gray-200 focus:ring-emerald-500" :value="old('value', $coupon->value)" required />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white  shadow-sm border border-gray-100 rounded-3xl">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="min_cart_value" name="min_cart_value" type="number" step="0.01" class="block w-full mt-1 rounded-xl border-gray-200 focus:ring-emerald-500" :value="old('min_cart_value', $coupon->min_cart_value)" required />
                                </div>
                                <div>
                                    <x-input-label for="max_uses" :value="__('Total Usage Limit')" class="text-xs font-bold uppercase text-gray-500 mb-2" />
                                    <x-text-input id="max_uses" name="max_uses" type="number" class="block w-full mt-1 rounded-xl border-gray-200 focus:ring-emerald-500" :value="old('max_uses', $coupon->max_uses)" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-white p-6 shadow-sm border border-gray-100 rounded-3xl">
                            ...
                            <div class="mt-8 space-y-3">
                                <button type="submit" class="w-full bg-emerald-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition duration-150 uppercase text-xs tracking-widest">
                                    Update Changes
                                </button>
                        ...
                        <div class="bg-emerald-600 p-6 rounded-3xl text-white shadow-lg shadow-emerald-100">
